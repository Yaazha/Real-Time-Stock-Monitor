...
# -------------------------------
ALPHA_VANTAGE_API_KEY = "Q84FUKMKA4UETESI"  # Replace with your API key
    ...
    if stock_symbol:
        ...
        if not data.empty:
            ...
            if alert_price > 0 and current_price >= alert_price:
                st.success(f"🚨 {stock_symbol} reached your target price: ${current_price:.2f}")
                if user_email:
                    st.info(f"Alert would be sent to: {user_email} (integration can be added)")
        else:
...
with tabs[2]:
    ...
    st.info("Alerts set in the 'Live Monitoring' tab will appear here when integrated with a backend or session state.")
    st.markdown("""
    - Currently, this tab is a placeholder.  
    - You can later store multiple alerts in session state or a small database to manage them.
    """)
